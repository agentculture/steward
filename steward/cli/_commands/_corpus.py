...
REPORT_BASENAME = "steward-suggestions.md"
REPORT_MARKER_PREFIX = "> Auto-generated by `steward doctor`"
PERFECT_PATIENT_BASENAME = "perfect-patient.md"
PRESCRIPTIONS_DIRNAME = ".prescriptions"

# Back-compat aliases retained for callers / tests that still use the old
# names. New code should reference the basename constants and let
# `_resolve_prescription_dir` (in doctor.py) build the absolute path.
PERFECT_PATIENT_RELPATH = Path("docs") / PERFECT_PATIENT_BASENAME
REPORT_RELPATH = Path("docs") / "steward" / REPORT_BASENAME
...
def render_perfect_patient(baseline: Baseline) -> str:
    ...
    lines: list[str] = [
        ...
        ">",
        "> This is a *prescription* — fresh output for the patient",
        "> (workspace) it was generated against. Doctor never overwrites a",
        "> committed `docs/perfect-patient.md` in place; instead each run",
        "> writes a new file under `<steward-root>/.prescriptions/<slug>/`.",
        "> Diff this against your committed baseline to see what the corpus",
        "> currently says, then copy any sections you want to ratchet.",
        "",
...
def write_prescription_report(target: Path, body: str) -> Path:
    """Write a per-sibling steward-suggestions report to ``target``.

    `target` is fully resolved by the caller — typically
    ``<steward-root>/.prescriptions/<slug>/<sibling-name>/steward-suggestions.md``.
    No "preserve unmanaged hand-edits" branch: prescriptions are
    derived output, regenerated fresh on every run, scoped to a
    gitignored scratch directory the user doesn't hand-edit.
    """
